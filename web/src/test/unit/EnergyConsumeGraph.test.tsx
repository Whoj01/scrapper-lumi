...
import { EnergyConsumeGraphCard } from '@/components/energyConsume/EnergyConsumeGraphCard'
...
describe('EnergyConsumeGraph suit test', () => {
  ...
  it("Should render graph card", () => {
    //ajeitar
    render(
      <EnergyConsumeGraphCard users={[user1, user2]} bills={[...user2Bills, ...user1Bills]} />
    )
    //atuar
  ...
  it("Should return the correct data", async () => {
    //ajeitar
    const { data, options } = useLineChart().renderEnergyConsumeGraph([user1, user2], [...user2Bills, ...user1Bills])

    const mockRenderEnergyConsumeGraph = vi.fn().mockReturnValue({
      data,
      options
    })

    vi.spyOn(useLineChartModule, 'useLineChart').mockReturnValue({
      renderEnergyConsumeGraph: mockRenderEnergyConsumeGraph,
    ...
    //afirmar
    expect(mockRenderEnergyConsumeGraph).toHaveBeenCalled()
    expect(mockRenderEnergyConsumeGraph).toHaveReturnedWith({ data, options })
  })
})