...
import { ValueTotalWithoutGDGraphCard } from '@/components/valuesEnergy/ValueTotalWithoutGDGraph'
...
describe('EnergyConsumeGraph suit test', () => {
  ...
  it("Should render graph card", () => {
    //ajeitar
    render(
      <ValueTotalWithoutGDGraphCard users={[user1, user2]} bills={[...user2Bills, ...user1Accounts]} />
    )
    //atuar
  ...
  it("Should return the correct data", async () => {
    //ajeitar
    const { data, options } = useLineChart().renderValueWithoutGDGraph([user1, user2], [...user2Bills, ...user1Accounts])

    const mockRenderValueWithoutGDGraph = vi.fn().mockReturnValue({
      data,
      options
    })

    vi.spyOn(useLineChartModule, 'useLineChart').mockReturnValue({
      renderValueWithoutGDGraph: mockRenderValueWithoutGDGraph,
    ...
    //afirmar
    expect(mockRenderValueWithoutGDGraph).toHaveBeenCalled()
    expect(mockRenderValueWithoutGDGraph).toHaveReturnedWith({ data, options })
  })
})